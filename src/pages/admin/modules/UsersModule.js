...
const UsersModule = ({ isLoggedIn, isAdmin, areUserStatusLoaded }) => {
    ...
    const headings = ['#', 'Nombre', 'DNI', 'Tipo de Usuario', 'Banneado', 'Estado'];

    useEffect(() => {

        const action = async () => {
            await apiClient.get(`/getAllUsers/${registerPerPageValue}`).then((response) => {
    ...
    if (areUserStatusLoaded) {
        ...
        return (
            <div className='container container-style' style={{ color: 'black' }}>
